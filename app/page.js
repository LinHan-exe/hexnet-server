...
export default function Home() {
  ...
  return (
    <div style={{ backgroundColor: '#0d1117', color: '#d1d4dc', minHeight: '100vh', padding: '40px', fontFamily: 'system-ui, sans-serif' }}>
      {/* EXPANDED WIDTH TO 1500px FOR MORE ROOM */}
      <div style={{ maxWidth: '1500px', margin: '0 auto' }}>
        ...
        {/* REMOTE CONTROL PANEL WITH BETTER SPACING */}
        <div style={{ backgroundColor: '#1e222d', padding: '20px', borderRadius: '8px', border: '1px solid #2b2b36', marginBottom: '30px', display: 'flex', gap: '20px', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            ...
            <button onClick={() => sendCommand({ status: 'start_requested' })} disabled={cmd.status === 'running' || cmd.status === 'start_requested'} style={{ backgroundColor: '#26a69a', color: 'white', border: 'none', padding: '12px 30px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', opacity: (cmd.status === 'running' || cmd.status === 'start_requested') ? 0.5 : 1 }}>
              START
            </button>
            
            <button onClick={() => sendCommand({ status: 'stop_requested' })} disabled={cmd.status === 'idle' || cmd.status === 'stopped'} style={{ backgroundColor: '#ef5350', color: 'white', border: 'none', padding: '12px 30px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', opacity: (cmd.status === 'idle' || cmd.status === 'stopped') ? 0.5 : 1 }}>
              STOP
            </button>
          </div>
        </div>
